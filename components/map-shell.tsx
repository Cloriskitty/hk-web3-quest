...
const HK_CENTER: [number, number] = [114.1700, 22.2800]
...
function createMarkerSprite(company: Company, active: boolean, dense: boolean) {
  ...
  if (company.mapSprite === "tombstone") {
    return createTombstoneSpriteMarker(company, active, dense)
  }
...
function createTombstoneSpriteMarker(company: Company, active: boolean, dense: boolean) {
  const wrapper = document.createElement("div")
  wrapper.style.display = "flex"
  wrapper.style.flexDirection = "column"
  wrapper.style.alignItems = "center"
  wrapper.style.filter = "grayscale(100%)"
  wrapper.style.opacity = active ? "1" : "0.6"
  wrapper.style.transition = "all 0.2s"
  wrapper.style.cursor = "pointer"

  const stone = document.createElement("div")
  const size = dense ? (active ? 38 : 28) : (active ? 48 : 34)
  stone.style.width = size + "px"
  stone.style.height = (size * 1.2) + "px"
  stone.style.background = "linear-gradient(180deg, #9e9e9e 0%, #616161 100%)"
  stone.style.borderRadius = (size / 2) + "px " + (size / 2) + "px 4px 4px"
  stone.style.display = "flex"
  stone.style.flexDirection = "column"
  stone.style.alignItems = "center"
  stone.style.justifyContent = "center"
  stone.style.border = "2px solid #424242"
  stone.style.boxShadow = active ? "0 0 12px rgba(0,0,0,0.5)" : "none"

  const rip = document.createElement("span")
  rip.textContent = "RIP"
  rip.style.fontSize = dense ? "8px" : "10px"
  rip.style.fontWeight = "900"
  rip.style.color = "#212121"
  rip.style.fontFamily = "serif"
  stone.appendChild(rip)

  const label = document.createElement("span")
  label.textContent = company.name.replace(/ \u2620\uFE0F/g, "")
  label.style.fontSize = dense ? "6px" : "7px"
  ...
  return (
    <div className="relative h-full min-h-0 overflow-hidden bg-[#cdb98b] lg:min-h-160">
      ...
      <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
        ...
        <a
          href="https://github.com/Cloriskitty/hk-web3-quest"
          target="_blank"
        ...
        <a
          href="https://sf-web3-quest.vercel.app"
          target="_blank"
          rel="noopener noreferrer"
          className="flex size-10 items-center justify-center border-[3px] border-[#342414] bg-[#f4ecd2] text-[#4c3926] shadow-[4px_4px_0px_#342414] transition-colors hover:bg-[#e7d8ae]"
          aria-label="Open SF Web3 Quest"
        >
          <span
